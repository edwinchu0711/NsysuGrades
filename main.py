...
def get_driver():
    options = Options()
    options.add_argument("--headless=new") # Render 必須開啟 headless
...
async def start_crawl(req: CrawlRequest):
    ...
    try:
        ...
        if req.task in ["score", "both"]:
            response_data["score_data"] = scrape_score(driver)
        
        if req.task in ["grades", "both"]:
            response_data["grades_data"] = scrape_historical_data_logic(driver) # 呼叫歷史成績邏輯
